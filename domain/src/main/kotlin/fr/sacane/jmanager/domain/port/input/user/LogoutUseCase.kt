...
data class LogoutCommand(val userId: UserId, val token: SessionToken) : Command<Nothing>
...
class LogoutService(
    ...
    override fun handle(command: LogoutCommand): Result<Nothing> {
        ...
        session.removeSession(command.userId, command.token)
        return success()
    }
}
