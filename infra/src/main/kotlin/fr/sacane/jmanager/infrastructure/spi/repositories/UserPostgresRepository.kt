...
interface UserPostgresRepository: CrudRepository<UserResource, Long> {
    ...
    @Query("SELECT user FROM UserResource user LEFT JOIN FETCH user.regularTransactions WHERE user.idUser = :id")
    fun findByIdWithRegularTransaction(id: Long): UserResource?
}