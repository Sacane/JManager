...
class ProblemDetailHandler {
    ...
    @ExceptionHandler(fr.sacane.jmanager.infrastructure.spi.NotFoundException::class)
    fun handleInfraNotFoundException(ex: fr.sacane.jmanager.infrastructure.spi.NotFoundException): ResponseEntity<ProblemDetail> {
        return buildResponse(
            status = HttpStatus.NOT_FOUND,
            title = "Not_Found error",
            detail = ex.message,
