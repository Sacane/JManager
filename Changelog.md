...
- **Fix — pages answered 401 when opened by URL, the email verification page included** (backend, no client change)
  - **Symptom, reproduced in production**: `/verify-email`, `/privacy`, `/terms`, `/consent`, `/force-password-change` and `/settings` returned a raw JSON 401 when opened from an email, a bookmark or a reload; `/login`, `/dashboard`, `/booklet`, `/tag` were fine. Navigating inside the app never showed it, because the Nuxt router does not ask the server. **The verification link in the welcome email could not be followed**, and the privacy policy could not be opened from outside the app.
  - **Root cause, verified**: a page reached the browser only if it was listed both in `SecurityConfig` and in `SpaController`. A page in neither was refused on the first request; a page whitelisted but unmapped — `/privacy` and the other legal pages — passed security, produced a 404, and that 404 was rendered through an internal ERROR dispatch to `/error`, which Spring Security 6 authorises like any request and which was not permitted. Permitting `/error` as an experiment turned exactly those four pages from 401 into 404.
  - **Fix**: one definition of a page, `SpaRoutes`, shared by both classes. A page is any path that is not under `api`, `actuator`, `error`, `_nuxt` or `assets` and has no dot in its first segment. There is no list to keep in step with `client/pages` any more: `/forgot-password` and `/reset-password` for UX-21 need no backend change, and an unknown path gets the client's own 404 screen.
  - **The reserved prefixes are security-relevant**: a mutation check reducing them made `/actuator/env` and `/actuator/heapdump` reachable, and missing `_nuxt` scripts answered as HTML, since a request mapping is consulted before the resource handlers. The guard test failed as it should.
  - **Tests**: `SpaRoutingTest` opens 18 paths anonymously with a browser-like `Accept` header. The four tests of `SpaControllerTest` called the same method with no argument and could not detect a missing page; they are replaced by one honest test.
  - **Found on the way, not fixed**: `JwtCookieAuthenticationFilter` answers **any** exception thrown further down the chain as a 401. My first attempt used a regex Spring refuses at match time, and every request, the home page included, answered 401. Noted in `docs/backlog/`.
  - **Report**: `docs/bugs/spa-pages-401-on-direct-load/REPORT.md`.

- **Fix — the sign-in rate limit was keyed on the reverse proxy, not on the visitor** (backend, one property)
  - **Symptom, observed in production**: the address recorded for a failed sign-in started with `172`, a private Docker range — nginx's. `LoginRateLimiter` keys on `HttpServletRequest.remoteAddr`, and without `server.forward-headers-strategy` that is the proxy for every request. All visitors shared one key: **five failed sign-ins from anyone locked sign-in for everyone** for 15 minutes, and anyone could trigger it deliberately with five requests.
  - **Fix**: `server.forward-headers-strategy=native`. nginx **appends** the address it saw to `X-Forwarded-For` (`$proxy_add_x_forwarded_for`), and Tomcat reads that header from the right, skipping trusted proxies — private ranges by default, which covers the Docker network — so a value a client puts on the left is ignored. Applied first in production through the `SERVER_FORWARD_HEADERS_STRATEGY` environment variable, which confirmed it: the key then started with the visitor's public address.
  - **Do not switch to `framework`**: it reads the header from the left and would honour a forged value, letting a client change its own key at will. A mutation check proves the guard — under `framework`, the forged-header test fails and the others pass.
  - **Tests**: `LoginRateLimitTest` runs against a real embedded server, since MockMvc bypasses the embedded server's handling of the header, and simulates nginx's appended header. `LoginRateLimiterTest` is new: the class had no test at all, nor did its 429 path.
  - **Trade-off to know about**: the global key also slowed down guessing one account's password, by accident. A per-client key no longer does, and no per-account limit exists. Noted in `docs/backlog/`, not solved here.

- **UI/UX workstream — lot 5: the dashboard** (PR #228, 3 items, frontend only)
  - **Context**: fifth batch of the review. UX-28 was a P2 item with no issue and no card; it was formalised at the start of its work. Two product decisions were taken on 19 September 2026 and are recorded in the issues: the three-zone layout with an analysis tab (option A), and the calendar month for the aggregated view.
  - **UX-44 — a real "Tous les comptes" mode**: the header could already read "Tous les comptes", but no such view existed — the selector listed individual booklets only, and that label appeared when a load had **failed**. The aggregated mode is now offered first, is remembered, totals every booklet's balance, and **uses the calendar month**, because monthly cycles are per booklet and an aggregation has no cycle of its own. A selected booklet keeps its own cycle. The account budget and the two quick actions needing one target booklet are hidden. Most of the aggregation already worked — the stats endpoints aggregate when no booklet id is sent — but **by accident**: the sentinel failed a UUID check and no cycle was found. Both rules are now written down rather than emergent.
  - **UX-18 — three zones and an analysis tab**: about fifteen blocks of equal visual weight made the page ask to be read in full. The overview answers one question per zone — **Où j'en suis**, **Ce qui arrive**, **Où part l'argent** — and the period comparison moves to an **Analyse** tab rather than lengthening the page. Blocks were **moved, not rewritten**, by a script checking each block's first line and title before touching it. Removed, each because it repeated or misrepresented a figure shown elsewhere: the four header pills (the exact date range joins the subtitle, which matters now that cycles are per booklet), **"Tags populaires"** — which listed the first six tags in API order under a title promising a ranking that did not exist — "Mes livrets" (reordering lives on the booklets page), the bottom stats banner, and the hardcoded "Objectif : 30 %". **A defect in the first version of this change was found while writing its validation cases**: the tab roles were announced without the behaviour they promise. A screen reader says "onglet", so the arrow keys must move between tabs with a single tab stop; the keyboard pattern is now complete.
  - **UX-28 — the dashboard on the design system**: 64 static `style` attributes held nine distinct token values. The colours were right — they already pointed at tokens — but **an inline style beats every class**, so no variant and no fix applied to a token could reach those blocks, and UnoCSS never saw them. There are none left. Seven shortcuts name what the page was copying: `block-title`, `kpi-label`, `kpi-value`, `kpi-hint`, `panel-sunken`, `text-label-strong`, `text-note`. **Eight block titles were written five ways**, in three sizes and two weights, for one role; they are unified on the majority form, so three titles come down from `text-xl` to `text-lg` — the one visible change. The backlog said 12 duplicated styles; recounting moved the effort from L to S. The two tag colour bindings stay bindings: they carry the colour the user picked.
  - **Fix — the monthly cycle card clipped its own control** (UX-41 follow-up, reported from the settings page): the "Fin" select was cut off by its card. The cycle box asked for 240px of selects plus the width of the longest date beside them, about 18px more than the card offers when the settings grid shows three columns — measured at 18px on a standalone reproduction. Two fixed columns become a wrapping row.
  - **Tests**: 77 files, 728 tests green. Two chart isolation tests assumed both charts on screen at once and now switch tab as a user would; two onboarding tests modelled a loading state the app cannot reach and now keep the fetch pending. The design-system guard reads the page source rather than the rendered markup, since half of this template sits behind a `v-if`.
  - **Manual validation**: `docs/technical/ux-design-review/VALIDATION_LOT_5.md` — 7 walkthroughs, 7 priority cases. This lot is almost entirely about layout and reading order, which happy-dom cannot see.

- **UI/UX workstream — lot 4: business pages** (PR #226, 8 items, frontend only)
  - **Context**: fourth batch of the review. UX-25 was planned here and reclassified full-stack before any code was written; it moved to lot 6. UX-41 and UX-50, P2 items with no issue yet, were formalised at the start of their work. UX-41 was merged with the existing Trello card "cycle mensuel du compte".
  - **UX-15 / UX-16 — regular transactions**: edit and delete actions visible on every row, desktop and mobile, and the delete confirmation names the entry. On mobile, tapping a card now selects it instead of opening the editor — without that, a phone could never build a selection and the bulk delete stayed unreachable. **UX-16's premise was partly false**: it claimed the booklet detail offered no obvious way to open a transaction on mobile, but the controls were already there. That page is untouched and pinned by regression tests.
  - **UX-19 — next occurrence and monthly commitment**: occurrence dates are computed in `utils/recurrence.ts`, which clamps a monthly day the target month does not have and measures day distances through UTC. The commitment **counts the occurrences that actually fall in the month** rather than normalising each entry: a weekly 10 € charge commits 50 € in a five-occurrence month, never 43.33 €. It covers the loaded page, which its tooltip states. The refactor pass caught a defect in the first version: a step-capped forward scan reported a daily charge started in 2005 as ended; the calculator now jumps to the candidate index. **Backlog**: `docs/backlog/recurrence-math-duplicated-client-side.md` — the rules now exist in TypeScript and in the Kotlin domain.
  - **UX-24 — linked booklets**: named on each row, and an unlinked entry says so. The reason a link action is unavailable sits on a wrapper element: a disabled button receives no pointer event, so a tooltip bound to it never fired.
  - **UX-20 — form errors on their field**: a shared `FieldError` component across five forms. **The password change mapping was wrong, not only incomplete**: a wrong current password is 403 (unhandled), 401 means the confirmation differs yet was announced as a wrong current password, an unchanged password is 400 (unhandled), and the 422 branch was unreachable. It is now keyed on the API's `errorKey`. The regular transaction dialog **failed silently** on an empty label. Sign-in stays form-level on purpose: naming the wrong field would reveal whether an account exists for an email.
  - **UX-26 — skeletons**: a shared `PageSkeleton` (dashboard, list, cards, form) replaces four loading treatments, announces a text alternative and honours `prefers-reduced-motion`. Skeletons are for first loads; reloads over content already shown, and actions such as saving, keep their spinner. It removed **three false empty states**: the booklet detail announced "Commencez par créer votre première transaction" while loading — with no indicator at all on mobile — and the admin table showed "Aucun utilisateur trouvé", never true for an admin page.
  - **UX-41 — monthly cycle per account**: each cycle is a fieldset named by its account, the name is no longer truncated, and a live preview shows the dates it covers. **The help text stated a false rule**: that the start day applies to the previous month. A month is anchored on its 15th, so that was wrong for start days 1 to 15 — verified by running the function. The rule is unchanged, since the dashboard depends on it; it is now shown instead of described. The preview reuses the dashboard's own function and is tested against it for all 31 start days.
  - **UX-50 — dashboard quick actions**: the three links duplicating the sidebar are replaced by adding a transaction, importing a CSV statement and creating a regular transaction, reusing the existing dialogs. The single-booklet actions name their booklet. The reload that follows exposed a latent defect: it shares the first load's scope, which drew the full-page skeleton, so every quick action would have wiped the dashboard. The skeleton is now reserved for the first load.
  - **UX-25 reclassified full-stack**: its issue said the balances endpoint provided the period's opening balance. `realSold` is the booklet's all-time persisted amount, date-blind. **Register opened**: `docs/technical/ux-design-review/FULLSTACK_PENDING.md` lists every full-stack item of the workstream — UX-21, 22, 23, 25, the backend gaps found while delivering, and the P2 full-stack items. Nothing in it is specified yet.
  - **Tests**: 74 files, 703 tests green. `FieldError` and `PageSkeleton` are kept unstubbed in `tests/setup.ts`, after `shallowMount` — which stubs globally registered components too — made two assertions vacuous during the lot.
  - **Manual validation**: `docs/technical/ux-design-review/VALIDATION_LOT_4.md` — 21 walkthrough sections, fourteen priority cases.
...
- **Audit + correctifs — frontières transactionnelles et fetch JPA** (suite du correctif `userOwnsBooklet`)
  ...
  - **B (moyenne-haute, domain) — N+1 sur la génération des transactions prévisionnelles** : `RegularTransactionGeneratorService.generateMissingPrevisionalTransactions` (appelée à chaque `GET /transactions` du mois courant) requêtait `findTransactionsByBookletYearAndMonth` à chaque itération de la boucle de génération (jusqu'à ~31 fois par transaction récurrente pour une récurrence quotidienne) au lieu d'une seule fois. La requête est désormais chargée une fois par transaction récurrente et propagée en paramètre dans toute la chaîne d'appel. Confirmé par un test comptant les appels réels (31 → 1), comportement métier inchangé.
  - **C (moyenne, domain) — N+1 sur l'import CSV** : `CsvDomainHelper.saveTransactions` appelait `bookletRepository.update(bookletParam)` une fois par ligne importée avec succès au lieu d'une seule fois après la boucle. Sorti de la boucle, appelé une seule fois. Confirmé par un test comptant les appels réels (5 lignes → 1 appel au lieu de 5).
  - **D (moyenne, domain) — écritures non atomiques** : `EditBookletService` et `SaveBookletService` faisaient plusieurs appels de repository séquentiels (vérification puis lecture puis écriture) sans les envelopper dans `executeInTransaction`, contrairement au pattern déjà suivi par `DeleteTransactionsByIdsService` — chaque appel ouvrait sa propre transaction, avec une vraie fenêtre de course sur la limite de 6 livrets par utilisateur (`SaveBookletService`). Les deux services enveloppent désormais tout `handle()` dans `executeInTransaction`. Confirmé par un test espionnant `UnitOfWorkTransactionProvider`. **Backlog** : `docs/backlog/audit-remaining-non-atomic-handlers.md` — 44 `handle()` au total n'utilisent pas ce pattern ; seuls ces deux cas concrets ont été traités, un inventaire complet reste à faire.
  - **E (moyenne, infra) — `@Transactional` manquant** : `RegularTransactionOperator.update()` n'avait pas `@Transactional`, contrairement à ses trois méthodes sœurs (`save`/`link`/`unlink`) dans le même fichier — piège latent (pas d'incident actuel, son unique appelant est déjà transactionnel). Annotation ajoutée par cohérence.
  - Suite complète (`./gradlew test`, domain + infrastructure + application) verte après chaque correctif.

- **Tri des transactions du livret entièrement côté serveur (Date, Libellé, Dépenses, Recettes)**
  - **Contexte** : `bfeddbc6` n'avait passé server-side que le tri par date. Les colonnes `Libellé` / `Dépenses` / `Recettes` restaient triées par PrimeVue côté client — donc uniquement sur la page chargée. Pire, le tri client de PrimeVue est *non contrôlé* et *collant* : après avoir trié par une de ces colonnes, tout rechargement backend trié par date (bouton « Date ») était aussitôt réordonné par PrimeVue → la flèche changeait, pas l'ordre. Rapport : `docs/bugs/date-sort-overridden-by-column-sort/`.
  - **Domain** : nouvel enum `TransactionSortField { DATE, LABEL, EXPENSE, INCOME }` ; `LoadTransactionsForBookletForAMonthQuery.sortField` nullable à côté de `sortDirection` (`direction` sans `field` ⇒ `DATE`, rétro-compatible). `sortForDisplay` généralisé et appliqué à toute la période avant pagination : `LABEL` insensible à la casse puis date ; `EXPENSE` / `INCOME` trient par montant dans le sens concerné et **rejettent l'autre sens en fin de liste, en ordre date, quelle que soit la direction** (reproduit le null-last de PrimeVue) ; confirmées et prévisionnelles interclassées. `directed()` et `orderByKindThenAmount()` extraits. 9 tests domaine.
  - **Application** : `GET {bookletID}/transactions` accepte `?sortField=` ; binding direct sur l'enum du domaine → 400 sur valeur inconnue via `ProblemDetailHandler`. 5 nouveaux tests REST.
  - **Client** : `AppTable` accepte `lazy` (défaut `false`), des props de tri contrôlées `sortField` / `sortOrder` et émet `sort`. Sur `booklet/[id].vue`, `AppTable` passe en `lazy` : PrimeVue ne trie plus rien, chaque en-tête de colonne (Date incluse, redevenue `sortable`) déclenche un rechargement backend. Suppression du bouton custom `#header-dateSort` / `toggleDateSort` / style `.date-sort-btn`. Un seul état `activeSort` (`{ field, direction }`, défaut `DATE`/`DESCENDING`) — **le dernier tri demandé gagne, sans fusion**. Le mode « tout le mois » trie le jeu complet en mémoire via `compareBySort` (miroir de la règle domaine). `useBooklet.findTransactionsByIdMonthAndYear(..., sortDirection?, sortField?)`. Champs `expenseSortValue` / `incomeSortValue` de `DisplayTransaction` supprimés (le tri client n'en a plus besoin). Tests : `AppTable.spec.ts` (lazy + tri contrôlé + ré-émission), bloc `pages/booklet/[id] sorting` réécrit (dont « un 2ᵉ tri de colonne remplace l'`activeSort` au lieu de fusionner »), `useBooklet.spec.ts`.
  - **Backlog** : `docs/backlog/regular-transactions-primevue-client-sort.md` — même défaut latent sur `regular-transaction/index.vue` (colonnes `sortable` PrimeVue en pagination server-side) ; `AppTable` reste `lazy: false` par défaut donc cette page est inchangée.

- **Correction : pagination invisible en bas de la page des transactions régulières (mode ordi/tablette)**
  - **Domain**: `userOwnsBooklet` chargeait tout le graphe (transactions + transactions récurrentes) de **tous** les livrets de l'utilisateur (`findBookletsForUser`) juste pour vérifier la propriété d'un seul id — introduit par `743c22c8` sur `DeleteTransactionsByIdsUseCase`, `EditTransactionUseCase`, `ConfirmPreviewTransactionUseCase`, `ConfirmVirtualTransactionUseCase`. Sa requête JPA fait `LEFT JOIN FETCH b.transactions LEFT JOIN FETCH b.regularTransactions` en une seule requête : `transactions` étant une `List` (bag) et non un `Set`, ce double fetch-join produit un produit cartésien qui **duplique** chaque transaction du livret une fois par transaction récurrente liée — y compris une transaction n'ayant aucun lien avec une récurrente, la duplication se produisant au niveau du livret. Comme cette requête s'exécute dans la **même session Hibernate** que l'appel `findBookletByIdWithTransactions` qui suit (les deux étant `@Transactional`), l'entité déjà en session — avec sa liste dupliquée — est réutilisée telle quelle. Résultat : `booklet.transactions.filter { ids.contains(it.id) }` matche N fois au lieu d'une seule, et l'égalité de taille attendue échoue → `TRANSACTION_NOT_FOUND` alors que la transaction existe bel et bien, dès qu'un livret a 2 transactions récurrentes liées ou plus (état parfaitement normal pour un compte actif). Nouveau port `BookletRepository.existsBookletForUser(userId, bookletId): Boolean`, sans aucun fetch-join, utilisé par `userOwnsBooklet` à la place de `findBookletsForUser`.
  - **Infrastructure**: `BookletJpaRepository.existsBookletForUser` (`SELECT COUNT(b) > 0 ... WHERE b.idBooklet = :bookletId AND b.owner.idUser = :userId`, aucun join), implémentée dans `BookletJpaRepositoryAdapter`. Reproduit par un test d'intégration contre un Postgres réel (Testcontainers) : livret + 3 transactions récurrentes + 1 transaction prévisionnelle créée manuellement → suppression échouait avant le correctif, réussit après. 2 nouveaux tests directs sur `existsBookletForUser`.
  - **Backlog**: `docs/backlog/retention-purge-fails-on-linked-regular-transactions.md` — trouvé en cours d'investigation, sans rapport avec ce correctif (le job nocturne de purge des comptes non consentis échoue quand un livret a encore des transactions récurrentes liées).

- **Correction : la sélection du livret sur le tableau de bord n'était pas mémorisée** (carte Trello #94 « garder en memoire le livret »)
  - **Client**: `selectedBookletId` (tableau de bord `pages/index.vue`) était un `ref` local — il repartait sur le premier livret à chaque démontage/remontage de la page (navigation vers un autre onglet puis retour). Remplacé par `useLocalStorage` (clé `dashboard.selectedBookletId.v1`, même convention que `BUDGET_STORAGE_KEY`). La sélection persistée n'est reprise que si elle correspond encore à un livret existant ; sinon repli sur le premier livret (cas d'un livret supprimé entre-temps). 2 nouveaux tests de page.

- **Tri des transactions par date sur l'ensemble des pages** (carte Trello #95 « triage des trans »)
  - **Domain**: nouvel enum `TransactionSortDirection` (`ASCENDING` / `DESCENDING`) et paramètre **nullable** `sortDirection` sur `LoadTransactionsForBookletForAMonthQuery`. Le tri est appliqué à la liste complète de la période **avant** `paginator.paginate`, jamais après. Le défaut est `null` — et non `ASCENDING` — parce que l'ordre actuel n'est pas un tri par date : les transactions confirmées sont concaténées avant les prévisionnelles. Un `ASCENDING` par défaut aurait donc changé l'affichage dès qu'une confirmée est postérieure à une prévisionnelle. Comparateur `date` puis `lastModified` extrait en constante (il était déjà écrit deux fois). 5 tests domaine.
  - **Domain — `BookletLoadingResult.orderedTransactions`**: le résultat exposait uniquement `currentTransactions` / `previsionalTransactions`, et **les trois** adaptateurs consommateurs reconstruisaient la liste par `currentTransactions + previsionalTransactions`, ce qui regroupait les confirmées avant les prévisionnelles et détruisait silencieusement l'ordre calculé par le domaine. Nouvelle propriété portant le contenu de la page dans l'ordre, avec défaut rétrocompatible ; contrat documenté sur le port pour éviter la rechute.
  - **Application**: `GET {bookletID}/transactions` accepte `?sortDirection=ASCENDING|DESCENDING`. Le binding direct sur l'enum du domaine fait remonter une valeur inconnue en `MethodArgumentTypeMismatchException`, déjà mappée en 400 par `ProblemDetailHandler` — aucune validation manuelle ajoutée. Les trois adaptateurs consomment désormais `orderedTransactions`. 4 nouveaux tests REST.
  - **Client**: la colonne « Date » n'est plus `sortable` au sens PrimeVue — le tri client ne réordonnait que la page chargée. L'en-tête est un bouton (`aria-sort`, `aria-label` explicite) qui bascule la direction, remet la pagination en page 1 et recharge depuis le backend ; la direction est conservée lors des changements de page, de mois et d'année. Les tris par date en mémoire de `loadBookletData`, du lazy-load mobile et de la création de transaction sont supprimés : le backend fait foi. Le mode « tout le mois » (endpoint report, jeu complet côté client) et le regroupement par jour du mobile suivent la même direction. Défaut `DESCENDING`, ce qui correspond à ce que l'utilisateur voyait déjà. 6 nouveaux tests (page + composable).
...
- **Correction : pagination invisible en bas de la page des transactions régulières (mode ordi/tablette)**
  - **Client**: sur `pages/regular-transaction/index.vue`, le `<AppTable>` du mode bureau n'avait pas les classes `flex-1 min-h-0` (présentes sur la page détail livret). Sans elles, la `DataTable` avec `scroll-height="flex"` se rendait à la hauteur totale de son contenu au lieu de rester un enfant flex borné : le pied de page contenant le `Paginator` et le sélecteur « Lignes par page » était poussé sous le conteneur `overflow-hidden` et n'apparaissait qu'en dézoomant. 1 test de page ajouté.
...
- **Restauration sélective des transactions prévisionnelles supprimées**
  - **Domain**: nouvelle query `FindRegenerableTransactionsUseCase` — liste, sans rien restaurer ni persister, les occurrences supprimées d'un livret pour un mois donné (`RegenerableTransaction` : label, montant, sens, tag, date). Nouvelle méthode `RegularTransactionGenerator.calculateOccurrencesIgnoringExclusions` : seul chemin de calcul qui contourne délibérément la grille d'exclusion, ce dont la query a besoin pour montrer ce qui reviendrait. `RegenerateDeletedPrevisionalTransactionsCommand` porte désormais `regularTransactionIds` (champ requis — le « restore all » aveugle disparaît) ; le service ne dé-exclut et ne régénère que `sélection ∩ réellement exclu`, une sélection vide étant refusée (`TRANSACTION_ENTRY_ERROR`). Prédicat d'exclusion partagé extrait en `RegularTransactionTrackerRepository.excludesMonth`. Contrôle de propriété (`userOwnsBooklet`) ajouté sur la query **et** la commande : un livret d'autrui est rapporté absent, pas interdit, pour ne pas offrir d'oracle d'énumération. 13 tests domaine.
  - **Application**: nouvel endpoint `GET {bookletID}/transactions/regenerable?month=&year=` (via `QueryBus`) renvoyant `List<RegenerableTransactionDTO>`. `POST {bookletID}/transactions/regenerate` accepte `{"regularTransactionIds": [...]}` avec `@Valid` + `@NotEmpty`. Longueur des identifiants bornée par `validateSelectedIdentifiers()` dans le controller : la contrainte de conteneur `List<@Size(max = 100) String>` n'est **pas** appliquée par Hibernate Validator ici — vérifié par un test dédié qui renvoyait 200 au lieu de 400. 7 nouveaux tests REST.
  - **Client**: nouveau `BookletRegenerateTransactionsDialog.vue` — le bouton « Régénérer » ouvre désormais une boîte de dialogue listant chaque transaction supprimée (label, date, montant) avec case à cocher par ligne et case « tout sélectionner », au lieu de tout restaurer en un clic. La sélection est indexée par `regularTransactionId` : les occurrences d'une même récurrence (hebdomadaire, quotidienne) basculent ensemble et la ligne l'annonce explicitement, parce que le backend exclut par couple (transaction récurrente, mois) et jamais par occurrence. Un échec de restauration laisse la boîte ouverte avec la sélection intacte. Couleurs issues exclusivement des tokens du design system. 15 nouveaux tests (composant, composable, page).
  - **Limite assumée**: l'exclusion étant suivie par `(regularTransactionId, mois)`, restaurer une occurrence d'une récurrence hebdomadaire ou quotidienne restaure tout le mois pour cette récurrence. Documenté dans le modèle, l'API et l'UI plutôt que masqué.
  - **Backlog**: `extract-recurrence-occurrence-calculator.md` (math de récurrence écrite 3×), `booklet-domain-uses-system-clock.md` (6 sites lisant l'horloge système en violation d'une règle MAJEURE), `regeneration-type-computed-in-controller.md` (règle métier dupliquée dans la couche application), `booklet-read-use-cases-skip-ownership-check.md` (même trou de propriété sur `FindBookletById`, `LoadTransactions`, `LoadBalances`), `dialog-dark-mode-bypasses-design-tokens.md` (palette hex codée en dur dans les dialogs).
